...
export class AdultHomePage {
  ...
  constructor() {
    forkJoin({ children: this.childrenService.getActiveChildren(), assignments: this.choresService.getAssignments(), rewardRedemptions: this.rewardRedemptionsService.get() }).subscribe({
      next: ({ children, assignments, rewardRedemptions }) => { this.children.set(children); this.assignments.set(assignments); this.rewardRedemptions.set(rewardRedemptions); },
  ...
  changeRewardRedemption(item: AdultRewardRedemption, action: 'approve' | 'cancel' | 'deliver'): void {
    ...
    this.rewardRedemptionsService.change(item.id, action, null).subscribe({
      next: (updated) => this.rewardRedemptions.update((items) => items.map((current) => current.id === updated.id ? updated : current)),
