...
const STATUS_LABELS: Record<ChildRedemptionCardStatus, string> = {
 ...
 * pastel-card shape as ChildRewardCard (including the same image
 * placeholder — a wish is for the same reward, so the same future photo
 * applies here too) — but with a status banner where the CTA button would
 * be, since there's nothing left for the child to do on this card. */
@Component({
