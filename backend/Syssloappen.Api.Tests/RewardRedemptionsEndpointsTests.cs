...
public sealed class RewardRedemptionsEndpointsTests : IDisposable
{
    ...
    private async Task RegisterLogin(HttpClient client, string name, string email) { Assert.Equal(HttpStatusCode.Created, (await client.PostAsJsonAsync("/api/auth/register", new RegisterAdultRequest { HouseholdName = name, Email = email, Password = Password })).StatusCode); await TestEmailConfirmation.ConfirmLatestAsync(client, factory.EmailSender, email); Assert.Equal(HttpStatusCode.OK, (await client.PostAsJsonAsync("/api/auth/login", new LoginRequest { Email = email, Password = Password })).StatusCode); }
}
