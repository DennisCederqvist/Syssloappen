...
**Poäng synliga för vuxna (`9a94e93`, PR #9)**

- `GET /api/children` returnerar `ChildWithPointsResponse` (`Id`, `Name`, `PhotoUrl`, `AvailablePoints`). `AvailablePoints` är intjänade poäng minus poäng reserverade av öppna belöningsförfrågningar, dvs samma tal som barnet ser (`ChildRewardsController.AvailablePoints`). Beräkningen ligger i `ChildPointBalances.AvailableAsync` och räknar för alla barn i ett par frågor (ingen N+1). `ChildResponse` (redigering av namn, foto) är avsiktligt oförändrad: poängen finns bara på listan.
- Frontend: `ChildWithPoints` (utökar `ChildSummary`) är returtypen från `ChildrenService.getActiveChildren()`. `AdultPageHeader` har valfria `points`/`pointsLabel` (stjärna + siffra under rubriken), som barnets vuxenprofil använder; Hem visar en `AdultBadge` i kortets `tileAction`-plats. Båda sidorna hämtar om barnlistan efter godkänd syssla och efter ändrad belöningsförfrågan.
- **Skuld:** poängformeln finns nu på tre ställen (`ChildRewardsController`, `ChildPointsController` och `ChildPointBalances`). Att låta barnets kod använda hjälparen vore en liten städning men rör barnvyn.
...
>
> - Enhetskopplings-/engångskodsmodalerna ("Koppla enhet" och "Visa enheter") i Barn och konton är ombyggda till `AdultSheet` på samma designsystem (PR #2, mergad som `cb55ef3`). Samma PR gjorde "Behöver godkännas"/"Belöningsönskningar" och (på barnprofilen) även "Aktuella sysslor"/"Senaste godkända sysslor" om till en viewport-styrd 1/2/3-kolumnsgrid (`grid-cols-1 sm:grid-cols-2 lg:grid-cols-3`) i stället för en enda full-bredd-kolumn, efter flera iterationer med användaren för att hitta en balanserad lösning som varken sträcker ut ensamma kort eller skapar bredd-mismatch mellan sektioner.
> - Inloggningssidans Vuxen-sida (delad komponent med Barn-fliken, som lämnades helt orörd) är ombyggd på samma designsystem, med referensmockup i `docs/adult-view-redesign/login page.png` (PR #3, mergad som `e1747ec`). Samma PR fixade tre förbisedda, redan existerande buggar som `npm run e2e` avslöjade (första riktiga körningen av hela svepet på ett tag): en pollution i "Lösenord"-fältets tillgängliga namn i "Lägg till barn" (samma mönster som Datum-fixen, löst med `role="group"`-omslag), att en syssla i Child-vyn tappade tangentbordsfokus efter rapportering (saknad `id`/`tabindex` i "Väntar på svar"-listan), samt en tidszonsbugg där tilldelningars förfallodatum jämfördes mot backendens UTC-"idag" medan tilldelningsformuläret räknar ut "idag" i webbläsarens lokala tid — kunde göra en tilldelning osynlig för barnet under den dagliga timmen då lokalt datum och UTC-datum skiljer sig åt. De fyra berörda "idag"-beräkningarna använder nu `TimeProvider.GetLocalNow()`; riktiga UTC-tidsstämplar (AssignedAt, sessioner osv.) är oförändrade.
>
> En publik, medvetet lekfull landningssida (separat från inloggningssidan) är beslutad som en framtida, separat arbetsdel — se `REQUIREMENTS.md` under "17. Möjliga framtida funktioner" → "Publik landningssida". Referensmockuper ligger i `docs/adult-view-redesign/landing page tablet.png` och `landing page phone.png`.
>
...
>
> Utöver timszonsfixen (som medvetet ändrade backend-logik för DueDate-jämförelser) har ingen ytterligare backendmodell, API-kontrakt eller migration ändrats i denna genomgång. Angular-produktionsbygge, `dotnet test` (129/129) och hela `npm run e2e`-svepet är gröna. Tre sedan tidigare kända och oförändrade testfel kvarstår i `ng test`, inte orsakade av denna genomgång:
>
...
3. **Frontend, "Hantera vuxna" och komplett "Bjud in vuxen".** Ny rad i Inställningar → `/vuxen/installningar/vuxna`. Efter användarens uppföljande önskemål slogs de två skärmarna ihop helt: den gamla separata "Bjud in vuxen"-sidan och dess egen Inställningar-rad är borttagna, och `/vuxen/bjud-in` omdirigerar nu till `/vuxen/installningar/vuxna`. Den enda kvarvarande sidan visar engångskod-skapandet överst och Householdets vuxna som fullbreda klickbara kort därunder (badge för huvudägare/inbjuden). Varje kort öppnar en detaljvy vars innehåll beror på vem som klickas: det egna kortet ger en redigerbar profil plus lösenordsbyte (se nedan) och — om man inte är huvudägare — en "Lämna familjen"-knapp med tvåstegsbekräftelse; huvudägarens kort, sett av någon annan, är skrivskyddat utan knapp; alla andra kort ger bara "Koppla bort" med samma tvåstegsbekräftelse som tidigare. Att koppla bort sig själv rensar lokalt sessionstillstånd och leder direkt till `/login` utan en extra utloggningsrundtur mot en session backend redan ogiltigförklarat.
4. **Vuxenprofil: namn och lösenord.** Efter samma önskemål lades tre nya, alltid valfria profilfält till på `ApplicationUser`: `FirstName`, `LastName`, `Nickname` (migrationen `AddAdultProfileFields`, ren tilläggsmigration utan backfill). De kan fyllas i redan vid registrering (både `/api/auth/register` och `/api/auth/register/invited`, samt motsvarande valfria fält i inloggningssidans "Skapa konto" och på `/acceptera-inbjudan`) och redigeras när som helst via det egna kortet i "Hantera vuxna" (`PUT /api/household/adults/me`). Ett separat `POST /api/household/adults/me/change-password` byter lösenord via Identitys inbyggda `ChangePasswordAsync`, som kräver nuvarande lösenord. Hem-sidans hälsning ("Välkommen, X!") visar nu ett beräknat `DisplayName` (användarnamn, annars förnamn, annars som tidigare den gamla e-postbaserade fallbacken) istället för att alltid klippa ut e-postens lokala del — vilket var särskilt illa för långa e-postadresser. Användarnamnet prioriteras överallt (Hem, "Hantera vuxna"-listan, profilsparning) eftersom det uttryckligen är tänkt att visas, medan för- och efternamn bara lagras.

Verifierat: 148/148 backendtester (Release), `ng build` rent, 53/56 frontendtester (samma 3 sedan tidigare kända och ofärgade testfel), hela `npm run e2e`-svepet grönt, samt två fullständiga körningar mot en riktig instans — en för själva vuxenhanteringen (ägarskydd, andra vuxens inbjudan, tvåstegsbekräftelsen, listuppdatering utan omladdning, omedelbar sessionsogiltigförklaring, e-postfrigöring) och en för den samlade sidan plus profil/lösenord (18 kontrollpunkter, inklusive att ett bytt lösenord verkligen fungerar för inloggning efter utloggning).
