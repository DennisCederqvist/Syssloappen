...
>
> **Nästa arbetsdel:** inte beslutat ännu. Två kandidater diskuteras — dels barnvyns motsvarande grafiska genomgång (som helt saknar en designspec eller referensmockuper ännu, till skillnad från Adult-vyn; skulle kräva ett separat designarbete innan kod), dels den redan beslutade, backend-först vuxenhanteringen (huvudägare på `Household`, listning av Householdets vuxna, säker bortkoppling, samt UI:t "Hantera vuxna" i Inställningar och en komplett "Bjud in vuxen"-sida). Se `REQUIREMENTS.md` under US-012 → "Planerad implementationsordning" för den senares exakta, gröna stegordning.
